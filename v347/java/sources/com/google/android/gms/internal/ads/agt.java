package com.google.android.gms.internal.ads;

import android.app.Activity;
import android.content.Context;
import android.text.TextUtils;
import android.util.DisplayMetrics;
import android.util.Log;
import android.view.MotionEvent;
import android.view.View;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.concurrent.Callable;
import java.util.concurrent.ExecutorService;
import java.util.concurrent.TimeUnit;

/* JADX INFO: loaded from: classes.dex */
public class agt extends ags {
    private static final Object s = new Object();
    private static final String t = agt.class.getSimpleName();
    private static boolean u = false;
    private static long v = 0;
    protected boolean r;
    private String w;
    private boolean x;
    private boolean y;
    private ahw z;

    protected agt(Context context, String str, boolean z) {
        super(context);
        this.r = false;
        this.x = false;
        this.y = false;
        this.w = str;
        this.r = z;
    }

    private static aht a(ahm ahmVar, MotionEvent motionEvent, DisplayMetrics displayMetrics) throws ahj {
        Method methodA = ahmVar.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=");
        if (methodA == null || motionEvent == null) {
            throw new ahj();
        }
        try {
            return new aht((String) methodA.invoke(null, motionEvent, displayMetrics));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahj(e);
        }
    }

    protected static synchronized void a(Context context, boolean z) {
        if (!u) {
            v = System.currentTimeMillis() / 1000;
            f3972a = b(context, z);
            u = true;
        }
    }

    private static void a(List<Callable<Void>> list) {
        ExecutorService executorServiceC;
        if (f3972a == null || (executorServiceC = f3972a.c()) == null || list.isEmpty()) {
            return;
        }
        try {
            executorServiceC.invokeAll(list, ((Long) aoo.f().a(aro.bD)).longValue(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(t, String.format("class methods got exception: %s", ahu.a(e)));
        }
    }

    private static ahm b(Context context, boolean z) {
        if (f3972a == null) {
            synchronized (s) {
                if (f3972a == null) {
                    ahm ahmVarA = ahm.a(context, "fFhi0cTZpyVQYwMwl7BCfa0fa6esmkRUaNr4ktfJYZ8=", "", z);
                    if (ahmVarA.b()) {
                        if (((Boolean) aoo.f().a(aro.bJ)).booleanValue()) {
                            ahmVarA.a("4o7tecxtkw7XaNt5hPj+0H1LvOi0SgxCIJTY9VcbazM/HSl/sFlxBFwnc8glnvoB", "RgSY6YxU2k1vLXOV3vapBnQwJDzYDlmX50wbm2tDcnw=", new Class[0]);
                        }
                        ahmVarA.a("1QeH3Cf7T53ayw17Ebbo9YTdhU+IFx0X5nCtC5gZQym4uicOVPXxYWmMK9k58i8n", "bHJRpFJ+2R5LAbYQUBDMyfYpLd1oiGixlpIqMJOBQPY=", Context.class);
                        ahmVarA.a("1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", Context.class);
                        ahmVarA.a("+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", Context.class);
                        ahmVarA.a("6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", Context.class);
                        ahmVarA.a("f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", Context.class);
                        ahmVarA.a("bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", Context.class);
                        ahmVarA.a("ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", Context.class);
                        ahmVarA.a("PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", Context.class);
                        ahmVarA.a("ryulhTIO44CH2QVINEPOtPkpQOSdbbq7HgisLeHH33VCs+TjwYWzYttvzJcQiI1k", "dnKLchXhJgI3U7L4th0PQJWe3DOIJJ7MUji5p7t5mjc=", MotionEvent.class, DisplayMetrics.class);
                        ahmVarA.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=", MotionEvent.class, DisplayMetrics.class);
                        ahmVarA.a("1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", new Class[0]);
                        ahmVarA.a("R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", new Class[0]);
                        ahmVarA.a("uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", new Class[0]);
                        ahmVarA.a("C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", new Class[0]);
                        ahmVarA.a("YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", new Class[0]);
                        ahmVarA.a("Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", new Class[0]);
                        ahmVarA.a("LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", Context.class, Boolean.TYPE, Boolean.TYPE);
                        ahmVarA.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", StackTraceElement[].class);
                        ahmVarA.a("j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", View.class, DisplayMetrics.class);
                        ahmVarA.a("WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", Context.class, Boolean.TYPE);
                        ahmVarA.a("wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", View.class, Activity.class, Boolean.TYPE);
                        ahmVarA.a("aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", Long.TYPE);
                    }
                    f3972a = ahmVarA;
                }
            }
        }
        return f3972a;
    }

    @Override // com.google.android.gms.internal.ads.ags
    protected final long a(StackTraceElement[] stackTraceElementArr) throws ahj {
        Method methodA = f3972a.a("nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=");
        if (methodA == null || stackTraceElementArr == null) {
            throw new ahj();
        }
        try {
            return new ahk((String) methodA.invoke(null, stackTraceElementArr)).f3998a.longValue();
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahj(e);
        }
    }

    @Override // com.google.android.gms.internal.ads.ags
    protected final zn a(Context context, View view, Activity activity) {
        List listAsList;
        zn znVar = new zn();
        if (!TextUtils.isEmpty(this.w)) {
            znVar.f5737b = this.w;
        }
        ahm ahmVarB = b(context, this.r);
        if (ahmVarB.b()) {
            try {
                aht ahtVarA = a(ahmVarB, this.f3973b, this.q);
                znVar.g = ahtVarA.f4019a;
                znVar.h = ahtVarA.f4020b;
                znVar.i = ahtVarA.f4021c;
                if (this.p) {
                    znVar.u = ahtVarA.f4022d;
                    znVar.v = ahtVarA.e;
                }
            } catch (ahj e) {
            }
            aao aaoVar = new aao();
            if (this.f3975d > 0 && ahu.a(this.q)) {
                aaoVar.m = Long.valueOf(ahu.a(this.k, this.q));
                aaoVar.n = Long.valueOf(ahu.a(this.n - this.l, this.q));
                aaoVar.o = Long.valueOf(ahu.a(this.o - this.m, this.q));
                aaoVar.r = Long.valueOf(ahu.a(this.l, this.q));
                aaoVar.s = Long.valueOf(ahu.a(this.m, this.q));
                if (this.p && this.f3973b != null) {
                    long jA = ahu.a(((this.l - this.n) + this.f3973b.getRawX()) - this.f3973b.getX(), this.q);
                    if (jA != 0) {
                        aaoVar.p = Long.valueOf(jA);
                    }
                    long jA2 = ahu.a(((this.m - this.o) + this.f3973b.getRawY()) - this.f3973b.getY(), this.q);
                    if (jA2 != 0) {
                        aaoVar.q = Long.valueOf(jA2);
                    }
                }
            }
            try {
                aht ahtVarB = b(this.f3973b);
                aaoVar.f3710a = ahtVarB.f4019a;
                aaoVar.f3711b = ahtVarB.f4020b;
                aaoVar.h = ahtVarB.f4021c;
                if (this.p) {
                    aaoVar.f3712c = ahtVarB.e;
                    aaoVar.e = ahtVarB.f4022d;
                    aaoVar.g = Integer.valueOf(ahtVarB.f.longValue() != 0 ? 1 : 0);
                    if (this.e > 0) {
                        aaoVar.f3713d = ahu.a(this.q) ? Long.valueOf(Math.round(this.j / this.e)) : null;
                        aaoVar.f = Long.valueOf(Math.round(this.i / this.e));
                    }
                    aaoVar.j = ahtVarB.i;
                    aaoVar.i = ahtVarB.j;
                    aaoVar.k = Integer.valueOf(ahtVarB.k.longValue() != 0 ? 1 : 0);
                }
            } catch (ahj e2) {
            }
            if (this.h > 0) {
                aaoVar.l = Long.valueOf(this.h);
            }
            znVar.K = aaoVar;
            if (this.f3975d > 0) {
                znVar.y = Long.valueOf(this.f3975d);
            }
            if (this.e > 0) {
                znVar.x = Long.valueOf(this.e);
            }
            if (this.f > 0) {
                znVar.w = Long.valueOf(this.f);
            }
            if (this.g > 0) {
                znVar.z = Long.valueOf(this.g);
            }
            try {
                int size = this.f3974c.size() - 1;
                if (size > 0) {
                    znVar.L = new aao[size];
                    for (int i = 0; i < size; i++) {
                        aht ahtVarA2 = a(f3972a, this.f3974c.get(i), this.q);
                        aao aaoVar2 = new aao();
                        aaoVar2.f3710a = ahtVarA2.f4019a;
                        aaoVar2.f3711b = ahtVarA2.f4020b;
                        znVar.L[i] = aaoVar2;
                    }
                }
            } catch (ahj e3) {
                znVar.L = null;
            }
            ArrayList arrayList = new ArrayList();
            if (ahmVarB.c() != null) {
                int iN = ahmVarB.n();
                arrayList.add(new aih(ahmVarB, znVar));
                arrayList.add(new aik(ahmVarB, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", znVar, iN, 1));
                arrayList.add(new aif(ahmVarB, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", znVar, v, iN, 25));
                arrayList.add(new aie(ahmVarB, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", znVar, iN, 44));
                arrayList.add(new aij(ahmVarB, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", znVar, iN, 12));
                arrayList.add(new ail(ahmVarB, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", znVar, iN, 3));
                arrayList.add(new aii(ahmVarB, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", znVar, iN, 22));
                arrayList.add(new aid(ahmVarB, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", znVar, iN, 5));
                arrayList.add(new ait(ahmVarB, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", znVar, iN, 48));
                arrayList.add(new aia(ahmVarB, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", znVar, iN, 49));
                arrayList.add(new aiq(ahmVarB, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", znVar, iN, 51));
                arrayList.add(new aip(ahmVarB, "nXKe8Ev09tosW935mj67BeJvnsHiQzgrotB4vyuZm+aJQgx77SXcf757aVGtnDOm", "yl+F7AaTCLci2yEfPOMTGPVVFLoqw9fKWSf4HTpN46M=", znVar, iN, 45, new Throwable().getStackTrace()));
                arrayList.add(new aiu(ahmVarB, "j5/WVRJsgiWNVIg0a6aXYQYocHm4rEEozf0LrIkPezct93ISwgyVBTLRIkEvztpn", "gCYklRkrI0pyKJ3j1u5Yl6Jj12a6PD1K39diuw7+f7o=", znVar, iN, 57, view));
                arrayList.add(new ain(ahmVarB, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", znVar, iN, 61));
                if (((Boolean) aoo.f().a(aro.bE)).booleanValue()) {
                    arrayList.add(new ahz(ahmVarB, "wxabFp8V9bGKut15duzoi2M8ew0t/HWGFw3NoPp8pFanKwQpdIx0WzpFbmJaLX9T", "grSrEy+Ml8J7boJNrN9uNiXpddsqwR6A9NVesV/hHT8=", znVar, iN, 62, view, activity));
                }
                if (((Boolean) aoo.f().a(aro.bG)).booleanValue()) {
                    arrayList.add(new air(ahmVarB, "aH422z6bUtRL57gOABo0AU5IQDHmWvjsQgQZk64bzhjXVYUZmajAsUz/rrnMYVsw", "UAmRxgiWCEqHprf42V+e65VvIvUfFXeXbaMy5B8vzwA=", znVar, iN, 53, this.z));
                }
            }
            listAsList = arrayList;
        } else {
            znVar.k = 16384L;
            listAsList = Arrays.asList(new aih(ahmVarB, znVar));
        }
        a((List<Callable<Void>>) listAsList);
        return znVar;
    }

    @Override // com.google.android.gms.internal.ads.ags
    protected final zn a(Context context, wj wjVar) {
        zn znVar = new zn();
        if (!TextUtils.isEmpty(this.w)) {
            znVar.f5737b = this.w;
        }
        ahm ahmVarB = b(context, this.r);
        if (ahmVarB.c() != null) {
            a(a(ahmVarB, znVar, (wj) null));
        }
        return znVar;
    }

    protected List<Callable<Void>> a(ahm ahmVar, zn znVar, wj wjVar) {
        int iN = ahmVar.n();
        ArrayList arrayList = new ArrayList();
        if (!ahmVar.b()) {
            znVar.k = 16384L;
            return arrayList;
        }
        arrayList.add(new aic(ahmVar, "LJD7GILw6ox/o3kYqhR5FF4FDylErrAaJ66Y8V8gtnNGF8DcetGJrYvTlHeK8kyR", "GKdCM/OywOIR4FMFS5XrDdTIpVvhoMFma/KSfRUdsbQ=", znVar, iN, 27, wjVar));
        arrayList.add(new aif(ahmVar, "1GzFbNgtZBsZ6xBrCbOvmw4GuyabWdtXgJqf+Aoik3E00oSTfDga4UiXvbD1YLYB", "1EiWqNFog84woyMiTveFonfDO5rsRz44QrKedC4xw+8=", znVar, v, iN, 25));
        arrayList.add(new aik(ahmVar, "uFEr0moMuWuurgrUpfSBjyUksFv6IqKMSuNqo2jCjRmu9JHDaSBf9LDKs2u/XE/B", "hDtufeJTxJSs5qQMCIPuyzc9ZPOtO/34yyf8Kf44rGo=", znVar, iN, 1));
        arrayList.add(new aim(ahmVar, "+8WN7RfVBrTKK5NXM+y9JA4Ygu2jPZokHNFGgulhBRZ32uCnyKWWGQNTE1weoQCM", "peILYfFejTn8xmazfpiZWeK2eaYjjxEBmAZ8bUD9PWU=", znVar, iN, 31));
        arrayList.add(new ais(ahmVar, "R1L5/7J00Fko/kDi6VDsBhCzpp4/7wzabnzdihUS7rPLvaXLFWrERChyvYYgVJRF", "4WtEOgM4wQ7wiw1OB3pMyAbaplBizcjE6LRBT/nrDgM=", znVar, iN, 33));
        arrayList.add(new aib(ahmVar, "1DzTSPdcTVkbB0cTQEaPo/MU1E1TGypt38nYJILt/8CcQbOuYjF6hLShUHnLb5uw", "dkCo8xze5sFyp7Msrb8F1JKjVIP7LutspgZPYQvsMlo=", znVar, iN, 29));
        arrayList.add(new aid(ahmVar, "6hrke/fCJ5tLCck7L6DvYxDB/To2PLUVGD3lbM1fdL2y9IScTypVRvrPFLLn4rDM", "+GAY853+DQRMYkeMG8Hbv1THvRgwf43Jh+3p4yQDqGw=", znVar, iN, 5));
        arrayList.add(new aij(ahmVar, "f8J+j+UhiTEjRF3iyDcFnM9Rm6LC/P+2PE2qff5S0gHSDPzrH4HNcnCaI+twvhbv", "MOBosqV6pejtL/WEgsHP3Y2p2WtApcEEa2XyqqRQCo0=", znVar, iN, 12));
        arrayList.add(new ail(ahmVar, "bYi99wWoqjOnUF4nnSWQsu9qLqARRsoauNez1uhJPws2YAlolLPh/xgKDnyI06MJ", "pjeIE8EsMfMk4SIebXHXNWKXTpK8v5vG6YqOxp1QH1M=", znVar, iN, 3));
        arrayList.add(new aie(ahmVar, "C4t4eP72HKoZsX62gBXGicLgvQbyKikYhXkvESjpMVAOEyUibmrLmpkJ5lZygzZE", "onbJNRechKgg7ilJYzLDEQ07M0uNlDZp05nZ4h6WJ40=", znVar, iN, 44));
        arrayList.add(new aii(ahmVar, "YOE4tLd8IhAbsNGoUEFHfu51/79yG0YlK4bMxi/e6gOIrtpqPN+TQgqpfYNBoiRy", "aFY444Zz3/sMYl+Q7uujofvlLWJ37651UMbaKJCFBnU=", znVar, iN, 22));
        arrayList.add(new ait(ahmVar, "ug+duzFNTlMWhYez8qKOR3bxkM8vLOPy9xVUBCCMTrplDFf/BnF2bNmA7hw0iYtI", "ogMtakENM6NAXJL7DCVTIbgawd5T3ZVhEaXugMAz+D4=", znVar, iN, 48));
        arrayList.add(new aia(ahmVar, "PRnAd1qZLf3/u2x/ZslcDq2vQzDbmFHkH/jSij536nFGwYo+g6o8PpQOJcDeuz++", "99eM4BYfXxOCBAhS2wQJENWQ4BMWrR/wKqrfHx1Ws8w=", znVar, iN, 49));
        arrayList.add(new aiq(ahmVar, "Q7gHsedaZvBKvqRLQGMIPKihViSMmXus6bBgU0lNdv+Pm9sSlP+Z+JltWG56lF1K", "mrdNgIRzsAu+cEIamFRkeCivuYIijUjvD76p38JyQec=", znVar, iN, 51));
        arrayList.add(new ain(ahmVar, "WiMYovSpRj6RF1r6zXON0hYVyLxgllUAYyMyyeBevdbJbdYcoBpd8nDjCwwclcSu", "vpq7z91A+vN3MIS65A+GwiFRgEFOFpQOdK5Z+mjW21w=", znVar, iN, 61));
        return arrayList;
    }

    @Override // com.google.android.gms.internal.ads.ags, com.google.android.gms.internal.ads.agr
    public final void a(View view) {
        if (((Boolean) aoo.f().a(aro.bG)).booleanValue()) {
            if (this.z == null) {
                this.z = new ahw(f3972a, view);
            } else {
                this.z.a(view);
            }
        }
    }

    @Override // com.google.android.gms.internal.ads.ags
    protected final aht b(MotionEvent motionEvent) throws ahj {
        Method methodA = f3972a.a("oT49Nu6m3oNdAQwbQzvwr/RE4TwZbR8mJKypakrI/GqBgyD2niyVic8N9UaqvU10", "LS3Cw7DlWSqpRsXrajoVsCxXOedCBnQXFZcLjXzD8so=");
        if (methodA == null || motionEvent == null) {
            throw new ahj();
        }
        try {
            return new aht((String) methodA.invoke(null, motionEvent, this.q));
        } catch (IllegalAccessException | InvocationTargetException e) {
            throw new ahj(e);
        }
    }
}
